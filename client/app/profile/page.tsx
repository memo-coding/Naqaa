      ...
      <main className="pt-32 max-w-4xl mx-auto px-8">
         <div className="relative mb-12">
            ...
            <div className={`flex flex-col md:flex-row items-center gap-8 ${dir === 'rtl' ? 'md:text-right' : 'md:text-left'}`}>
               ...
               <div className="flex-1">
                  <h1 className="text-4xl font-black font-headline uppercase text-on-surface tracking-tight">{user.name}</h1>
                  <p className="text-on-surface-variant font-medium mt-1">{user.email}</p>
                  <div className="flex flex-wrap gap-2 mt-4">
                     <span className="px-3 py-1 bg-primary/10 text-primary rounded-xl text-[10px] font-black uppercase tracking-widest border border-primary/10">
               ...
               <button onClick={handleEditClick} className="px-8 py-4 bg-surface-container-high hover:bg-white/5 border border-white/5 text-xs font-bold uppercase tracking-widest rounded-2xl transition-all">
         ...
         <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <section className="glass-panel p-8 rounded-2xl border border-primary/10">
               <h3 className="text-xs font-black uppercase tracking-[0.2em] mb-6 flex items-center gap-2 text-primary">
               ...
               <div className="space-y-4">
                  {ordersLoading ? (
                    ...
                    <div className="text-center py-8 opacity-40">
                      ...
                      <p className="text-xs font-bold uppercase tracking-widest">{t('profile_no_orders') || 'No rituals recorded yet'}</p>
                    </div>
                  ) : (
                  dbOrders.slice(0, 10).map((order) => (
                    ...
                    >
                       <div className={dir === 'rtl' ? 'text-right' : 'text-left'}>
                          <p className="font-bold text-sm tracking-tight text-on-surface">Order #ORD-{order.id.toString().substring(0, 6).toUpperCase()}</p>
               ...
               <button className="w-full mt-6 py-4 text-[10px] font-black uppercase tracking-widest text-on-surface-variant hover:text-primary transition-all">
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-background/80 backdrop-blur-sm">
          <div className="bg-surface-container border border-primary/20 rounded-3xl p-8 max-w-md w-full relative">
            ...
            <form onSubmit={handleSaveProfile} className="space-y-4 text-left" dir={dir === 'rtl' ? 'rtl' : 'ltr'}>
              ...
              <div>
                <label className="block text-xs font-bold uppercase tracking-widest text-on-surface-variant mb-2">New Password (Optional)</label>
                <input 
                  type="password" 
                  value={editPassword}
              ...
              <button 
                ...
                className="w-full mt-4 py-4 bg-primary text-on-primary-container font-black text-sm uppercase tracking-widest rounded-xl hover:bg-primary/90 transition-colors disabled:opacity-50"
              >
          ...
          <div className="bg-surface-container-highest border border-primary/20 w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
            <div className="p-10 border-b border-primary/10 flex justify-between items-start relative">
               <div className="absolute top-0 right-0 w-32 h-32 bg-primary/5 rounded-xl blur-3xl -z-10"></div>
                <div>
                   <h3 className="text-3xl font-black font-headline tracking-tighter text-primary uppercase mb-1">Order #ORD-{selectedOrder.id.toString().substring(0, 6).toUpperCase()}</h3>
                   <p className="text-xs text-on-surface-variant/60 font-bold uppercase tracking-widest">{new Date(selectedOrder.created_at).toLocaleDateString()} / Placed on {new Date(selectedOrder.created_at).getDate()}</p>
                </div>
            ...
            <div className="p-10 max-h-[65vh] overflow-y-auto custom-scrollbar space-y-10">
                     ...
                     <div className="space-y-1">
                        ...
                        {selectedOrder.shipping_company_name && (
                           <div className="mt-4 p-3 bg-primary/10 rounded-xl border border-primary/20 inline-flex items-center gap-2">
                              ...
                              <span className="text-[10px] font-black uppercase tracking-widest text-primary">{dir === 'rtl' ? 'شركة الشحن' : 'Courier'}: {selectedOrder.shipping_company_name}</span>
                           </div>
                        )}
                     </div>
                  </div>
                  <div className={dir === 'rtl' ? 'text-right' : 'text-left'}>
                     <p className="text-[10px] font-black uppercase tracking-[0.2em] text-primary mb-4 flex items-center gap-2 justify-end">
                     ...
                     <p className="text-on-surface-variant text-xs font-bold text-right mt-1 tracking-widest">{selectedOrder.customer_phone}</p>
                  </div>
               </div>
               ...
               <div className="bg-surface-container rounded-3xl p-6 border border-outline-variant/10 flex items-center justify-between group flex-row-reverse">
                  <div className="flex items-center gap-4">
                     <div className="text-right">
                        <p className="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest">{dir === 'rtl' ? 'طريقة الدفع' : 'Payment Method'}</p>
                        <p className="font-black text-on-surface uppercase tracking-tight">
               ...
               {/* Order Items */}
               <div>
                  <p className="text-[10px] font-black uppercase tracking-[0.3em] text-primary mb-6 text-center">{dir === 'rtl' ? 'أصناف العينات' : 'Constituent Specimens'}</p>
            ...
            <div className="p-10 bg-surface-container-high/50 flex flex-col md:flex-row items-center justify-between gap-6 border-t border-primary/10">
               {selectedOrder.status === 'shipped' && (
                 <button 
                   ...
                   className="w-full md:w-auto px-8 py-4 bg-primary text-on-primary-container font-black text-[10px] uppercase tracking-[0.2em] rounded-2xl shadow-xl hover:shadow-primary/20 transition-all flex items-center justify-center gap-3 animate-pulse"
                 >
                ...
                <div className="text-center md:text-right w-full md:w-auto ml-auto">
                   <p className="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest mb-1">{dir === 'rtl' ? 'إجمالي القيمة' : 'Total Value'}</p>
                   <p className="text-4xl font-black font-headline text-on-surface tracking-tighter">${selectedOrder.total_amount.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}</p>
                </div>
            </div>
          </div>
        </div>
