...
export default function TrackOrderPage() {
  ...
  return (
    <div className="min-h-screen bg-[#030804] text-on-surface font-body overflow-hidden transition-colors duration-500" dir={dir}>
      {/* Heavy atmosphere */}
      ...
      <main className="pt-40 max-w-2xl mx-auto px-8">
        <div className="text-center mb-16 space-y-4">
          <span className="text-[10px] font-black uppercase tracking-[0.4em] text-secondary">{t('track_status_label') || 'RITUAL LOGISTICS'}</span>
          <h1 className="text-5xl md:text-7xl font-black font-headline uppercase tracking-tight">{t('nav_track') || 'Track Ritual'}</h1>
        ...
        <form onSubmit={handleTrack} className="glass-panel p-2 rounded-3xl border border-primary/20 flex gap-2 relative">
          <input 
            ...
            className={`flex-1 bg-transparent border-none ${dir === 'rtl' ? 'pr-8 pl-4' : 'pl-8 pr-4'} py-5 text-sm font-black tracking-widest outline-none placeholder:text-on-surface-variant/30 uppercase`}
            placeholder={t('track_placeholder') || 'ORD-XXXXXX'}
          />
          <button 
            type="submit"
            disabled={isLoading}
            className="px-8 py-4 bg-primary text-on-primary-container font-black uppercase tracking-widest rounded-2xl shadow-[0_0_30px_var(--accent-glow)] hover:shadow-[0_0_50px_var(--accent-glow)] transition-all active:scale-[0.98] disabled:opacity-70"
          >
        ...
        {errorMsg && (
          <div className="mt-6 text-center animate-in fade-in zoom-in duration-300">
            <p className="text-error font-bold uppercase tracking-widest text-sm flex items-center justify-center gap-2">
            ...
            <div className="glass-panel p-10 rounded-[2.5rem] border-secondary/20 relative overflow-hidden group">
               ...
               <div className="flex flex-col md:flex-row justify-between items-center gap-8 border-b border-white/5 pb-8 mb-8">
                  <div className={dir === 'rtl' ? 'text-right' : 'text-left'}>
                     <p className="text-[10px] font-black uppercase tracking-[0.2em] text-secondary opacity-80 mb-1">{t('track_id_label') || 'Identification'}</p>
                     <h3 className="text-xl font-black font-headline uppercase tracking-tighter">
                       #ORD-{ (timeline?.id || orderId).toString().toUpperCase().replace('ORD-', '').replace('#', '').trim().substring(0, 6) }
                     </h3>
                  </div>
                  <div className="px-6 py-3 bg-secondary/10 border border-secondary/20 rounded-xl text-[10px] font-black uppercase tracking-widest text-secondary shadow-[0_0_20px_rgba(47,248,1,0.2)]">
                  ...
                  {[
                    ...
                    <div key={i} className={`flex gap-8 items-start relative ${step.active ? 'text-on-surface' : 'text-on-surface-variant/40'}`}>
                       ...
                       <div className={dir === 'rtl' ? 'text-right' : 'text-left'}>
                          ...
                          <p className="text-[10px] font-bold mt-1 tracking-tighter">{step.date}</p>
                       </div>
                    </div>
                  ))}
               </div>
               ...
               <div className="mt-12 p-6 bg-white/5 rounded-2xl border border-white/5 flex items-center gap-6 group hover:border-secondary/30 transition-all transition-transform hover:scale-[1.01]">
                  ...
                  <div className="flex-1">
                     <p className="text-[10px] font-black uppercase tracking-[0.2em] text-secondary mb-1">Current Node</p>
                     <p className="text-xs font-bold leading-tight uppercase">Biolab Hub 04 - Sector C7 <br/><span className="text-[10px] opacity-40 font-medium tracking-tighter uppercase">Atmospheric stabilization required for transit</span></p>
                  </div>
               </div>
            </div>
          </div>
        ...
        <div className="mt-20 text-center pb-20">
           <Link href="/shop" className="text-[10px] font-black uppercase tracking-[0.3em] text-on-surface-variant hover:text-primary transition-all flex items-center justify-center gap-2 group">
